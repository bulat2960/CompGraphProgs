...
vector<vector<double>> getCameraMatrix(Vec3f cameraPosition, Vec3f figurePosition, const Vec3f up = Vec3f(1, 0, 0))
{
    Vec3f z = (cameraPosition - figurePosition).normalize();
    Vec3f y = (z ^ up).normalize();
    ...
    for (int i = 0; i < 3; i++)
    {
        matrix.push_back({x[i], y[i], z[i], 0});
    }
    matrix.push_back({0, 0, 0, 1});
    return matrix;
}
...
int main()
{
    ...
    v.update(getCameraMatrix(Vec3f(1, 1, 3), Vec3f(0, 0, 0)));
